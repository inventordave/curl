...
#if OPENSSL_VERSION_NUMBER >= 0x03000000fL && !defined(OPENSSL_IS_BORINGSSL)
...
#if defined(USE_OPENSSL_ENGINE) || OPENSSL_VERSION_NUMBER >= 0x03000000fL
/*
...
static CURLcode ossl_set_provider(struct Curl_easy *data, const char *provider)
{
  ...
  if(OSSL_PROVIDER_available(NULL, provider)) {
    ...
    /* FIXME there should probably be a way to get a link to the provider.
     * The 0x1 pointer just makes things work, but we do not use the pointer
     * for anything now. */
    data->state.provider = (void *)1;
    return CURLE_OK;
  }
  if(!pkcs11_provider_failed) {
    ...
    data->state.provider = pkcs11_provider;
  }
